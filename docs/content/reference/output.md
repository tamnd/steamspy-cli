...
describes what users get. Pick a format with `-o`, or let steamspy choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
steamspy <command> -o table   # aligned columns for reading
steamspy <command> -o jsonl   # one JSON object per line, for piping
steamspy <command> -o json    # a single JSON array
steamspy <command> -o csv     # spreadsheet friendly
steamspy <command> -o tsv     # tab-separated
steamspy <command> -o url     # just the URL column
steamspy <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
steamspy <command> --fields id,title,url
```
...
```bash
steamspy <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
steamspy <command>            # a table, because this is a terminal
steamspy <command> | wc -l    # JSONL, because this is a pipe
```
